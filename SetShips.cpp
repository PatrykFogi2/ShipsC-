...
void SetShips::Playerplay() {
  int row, col ; int x; int i = 4;  int z=1;
 
  Direct kierunek;
   cout <<"Zaczynamy od rozłożenia statków na planszy" << endl; 
    Board.display();
...
void SetShips::ComputerPlay() {
 ...
 int row, col ; int x; int i = 4;  int z=1;  Direct kierunek;
while(i != 0) {
  
  for( int j=0; j <z; j++) {
         
          int row = AI.Random(); int col = AI.Random(); int x =rand()%2 ;
